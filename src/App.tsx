...
type ScaleNumber = 1 | 2 | 3 | 4 | 5 | 6 | 7;
...
const SCALE: Record<ScaleNumber, string> = {
...
function buildChoices(correctNumber: ScaleNumber): string[] {
  const correct = SCALE[correctNumber];
...
export default function App() {
  ...
  function advance(currentResults: QuestionResult[]) {
    ...
    setQuestion({ number, choices: buildChoices(number) });
    setFeedback(null);
    setSelected(null);
  }
  ...
  function handleAnswer(note: string) {
    ...
    const correct = note === SCALE[question!.number];
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 gap-8">
        ...
        <div className="text-9xl font-bold">{question.number}</div>

        <div className="grid grid-cols-2 gap-3 w-full max-w-xs">
          {question.choices.map(note => {
            const isCorrect = note === SCALE[question.number];
        ...
        <div className="h-7">
          {feedback && (
            <p className={`text-lg font-medium ${feedback === 'correct' ? 'text-green-400' : 'text-red-400'}`}>
              {feedback === 'correct' ? 'Correct!' : `It's ${SCALE[question.number]} — you'll see this again`}
            </p>
          )}
        </div>
      </div>
